...
#define ArraySize_Pini 3      /* 設定n*n方陣點的初始固定位置(n為奇數)，使初始搜尋更加均勻，但注意Population須足夠!! */ 
          ...
          double Mean_bef[], int Object_point[], int Displacement[], double CoefValue[])
{
	...
	for (i=0;i<Population;i++) /* i: 點編號 */ 
	{
		...
		else
		{
			...
			Pi_v_ini = (int)Pi[i][1];
			Cost_initial = Cost_function(Pi_u_ini, Pi_v_ini, Object_point,\
			                             img_aft, img_aft_sub, img_bef_sub, Mean_bef);  /* 注意!! 這裡不受Dimension而變化，若要更換維度請注意!!!*/ 
