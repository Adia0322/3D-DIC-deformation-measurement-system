...
base_path = os.path.join('.', 'image', 'target', str(plane))
file_name = f"{CF_user.TEST_LOAD}kg_image1.jpg"
img_1B_address = os.path.join(base_path, 'camera1', file_name)
img_2B_address = os.path.join(base_path, 'camera2', file_name)
...
# Reload the image (to remove the coordinates pattern).
img_1B = cv.imread(str(img_1B_address))
...
anaMat = np.zeros((int(np.sqrt(CF_user.TEST_POINT_ARRAY)),int(np.sqrt(CF_user.TEST_POINT_ARRAY))), dtype=int)
# scan size 
Scan_1B2B = 31
Scan_1B1A = 31
Scan_2B2A = 31
...
for P in range(-side_len_half,side_len_half+1,1):
    for L in range(-side_len_half,side_len_half+1,1):  
        C1_B_x = int(CF_user.TEST_INTERVAL*L + v1)
        C1_B_y = int(CF_user.TEST_INTERVAL*P + u1)
        ...
        # 從全圖插值表找插值(用於1B2B搜尋)
        Length_1B2B = int(0.5*(CF_user.TEST_SUBSET_SIZE_2B2A-1)+0.5*(Scan_1B2B-1))
...
for ImgNum in range(1,2,1):
    ...
    # 讀入影像
    img_1A_adress = './images/'+CF_user.TEST_IMG_DIR+'/'+str(plane)+'/camera1/cal_'+\
                      str(CF_user.TEST_LOAD)+'_'+str(CF_user.TEST_LOAD)+'kg_0cm.image'+str(ImgNum)+'.jpg'
    img_2A_adress = './images/'+CF_user.TEST_IMG_DIR+'/'+str(plane)+'/camera2/cal_'+\
                      str(CF_user.TEST_LOAD)+'_'+str(CF_user.TEST_LOAD)+'kg_0cm.image'+str(ImgNum)+'.jpg'
    ...
    # 計算1A影像插值係數
    # Length
    Length = int(0.5*(CF_user.TEST_SUBSET_SIZE_1B1A-1)+0.5*(Scan_1B1A-1)+20)   #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Length_1B1A = int(0.5*(CF_user.TEST_SUBSET_SIZE_1B1A-1)+0.5*(Scan_1B1A-1))
    Length_2B2A = int(0.5*(CF_user.TEST_SUBSET_SIZE_2B2A-1)+0.5*(Scan_2B2A-1))
    ...
    for P in range(-side_len_half,side_len_half+1,1):
        for L in range(-side_len_half,side_len_half+1,1):
            ...
            Length = int(0.5*(CF_user.TEST_SUBSET_SIZE_1B1A-1)+0.5*(Scan_1B1A-1))
            ...
            C1_A_x, C1_A_y, Coef_1B1A =\
                PSO_ICGN_1B1A.Calculate_1B1A(img_1B_new_gray, img_1A_new_gray,\
                                             C1_B_x, C1_B_y, CF_user.TEST_SUBSET_SIZE_1B1A, Scan_1B1A,\
            ...
            C2_A_x, C2_A_y, Coef_2B2A =\
                PSO_ICGN_2B2A.Calculate_2B2A(img_2B_new_gray, img_2A_new_gray,\
                                             C2_B_x, C2_B_y, CF_user.TEST_SUBSET_SIZE_2B2A, Scan_2B2A,\
...
print('Average time per point: ', TOTALtime/(CF_user.TEST_POINT_ARRAY*10))
print('Average dis:',disSum/(ImgNum*side_len*side_len))
print("End")
